...
Rcpp::List save_results(int N, int no_dims, const std::vector<double>& P_stores, const std::vector<double>& Y, const std::vector<double>& costs, const std::vector<double>& itercosts,
...
Rcpp::List Rtsne_cpp(NumericMatrix X, int no_dims, double perplexity, 
    ...
    std::vector<double> P_stores(N * N), Y(N * no_dims), costs(N), itercosts(static_cast<int>(std::ceil(max_iter/50.0)));
    ...
    if (no_dims==1) {
      ...
      tsne.run(data, N, D, Y.data(), distance_precomputed, costs.data(), itercosts.data(), P_stores.data());
    } else if (no_dims==2) {
    ...
    return Rcpp::List::create(Rcpp::_["Y"]=Rcpp::NumericMatrix(no_dims, N, Y.data()), 
            ...
            Rcpp::_["P"]=Rcpp::NumericMatrix(N, N, P_stores.data()));
}
...
Rcpp::List Rtsne_nn_cpp(IntegerMatrix nn_dex, NumericMatrix nn_dist, 
    ...
    if (no_dims==1) {
      TSNE<1> tsne(perplexity, theta, verbose, max_iter, init, stop_lying_iter, mom_switch_iter, 
              momentum, final_momentum, eta, exaggeration_factor, num_threads);
      tsne.run(nn_dex.begin(), nn_dist.begin(), N, K, Y.data(), costs.data(), itercosts.data(), P_stores.data());
    } else if (no_dims==2) {
      TSNE<2> tsne(perplexity, theta, verbose, max_iter, init, stop_lying_iter, mom_switch_iter, 
              momentum, final_momentum, eta, exaggeration_factor, num_threads);
      tsne.run(nn_dex.begin(), nn_dist.begin(), N, K, Y.data(), costs.data(), itercosts.data(), P_stores.data());
